...
public class SignIdentityProver implements IdentityProver {
	...
	public SignIdentityProver(Storage storage) {
	...
	public boolean isValid(String input, String username) {
		...
		try {
			mapper.readValue(input, SignIdentityProof.class);
		} catch (IOException e) {
	...
	public void addAttributes(String input, String username) {
		ObjectMapper mapper = new ObjectMapper();
		SignIdentityProof proof;
		try {
			proof = mapper.readValue(input, SignIdentityProof.class);
